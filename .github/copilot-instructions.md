...
5. Записи идут **от новых к старым** (первый элемент массива — самая свежая версия)
6. **При мерже `dev → master`** — сменить `status: 'unreleased'` → `'released'`
7. **После мержа** — ничего создавать не нужно (новая unreleased-секция появится при следующем изменении)
8. При создании **нового прототипа** — создать `changelog.data.ts` с версией `1.0`
9. Добавить новый slug в `switch` компонента `changelog-button.component.ts`
