...
import { FalconModalType, FalconContextType, ScenarioStep, AvailableRobot, RegistrationState } from '../types';
import {
  MOCK_CURRENT_ORDER,
  MOCK_TABLES,
...
import { CurrentOrder, FalconNotification, OrderTable } from '../types';
...
import { FalconLoadingDialogComponent } from '../components/dialogs/loading-dialog.component';
import { FalconSuccessDialogComponent } from '../components/dialogs/success-dialog.component';
import { FalconErrorDialogComponent } from '../components/dialogs/error-dialog.component';
...
@Component({
  selector: 'app-falcon-pos-screen',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
    ...
    FalconLoadingDialogComponent,
    FalconSuccessDialogComponent,
    FalconErrorDialogComponent,
  ...
  template: `
    <div class="min-h-screen bg-[#2d2d2d] flex flex-col relative" style="font-family: Roboto, sans-serif;">
      ...
      <ng-container *ngIf="currentContext === 'order'">
        ...
        <!-- Панель кнопок Falcon — контекст «Из заказа»: кнопка «Дополнения» + 4 кнопки действий -->
      ...
      <ng-container *ngIf="currentContext === 'main'">
        ...
        <!-- Панель кнопок Falcon — контекст «Главный экран»: 1 кнопка «Дополнения» v1.8 -->
      ...
      <falcon-send-dish-pickup-notify
      ...
      <!-- М9: Loading -->
      <falcon-loading-dialog
        [open]="activeModal === 'loading'"
        [message]="loadingMessage"
      ></falcon-loading-dialog>

      <!-- М10: Успех -->
      <falcon-success-dialog
          ...
          <div class="p-4">
            <div class="grid grid-cols-3 gap-2" style="grid-template-rows: repeat(6, 1fr);">
              <button (click)="onPluginRobotStatus()"
                ...
                <span class="text-center leading-tight px-1">Falcon: Статус роботов</span>
              </button>
              <button (click)="onPluginFalconCommands()"
                class="h-16 text-sm bg-white text-black hover:bg-gray-100 border border-gray-300 rounded font-medium transition-colors flex flex-col items-center justify-center gap-1">
                <span class="text-center leading-tight px-1">Falcon: Команды</span>
              </button>
      ...
      <div *ngIf="activeModal === 'falcon_commands'"
           class="fixed inset-0 z-50 flex items-center justify-center bg-black/60"
           (click)="closeDialog()">
        <div class="bg-[#3a3a3a] rounded-lg w-full max-w-md mx-4 overflow-hidden animate-slide-up"
             (click)="$event.stopPropagation()">
      ...
      <div *ngIf="activeModal === 'plugins_menu_order'"
        ...
        <div class="bg-[#3a3a3a] rounded-lg w-full max-w-md mx-4 overflow-hidden animate-slide-up"
          ...
          <div class="p-4">
            <div class="grid grid-cols-3 gap-2" style="grid-template-rows: repeat(6, 1fr);">
              <button (click)="onPluginFalconCommandsOrder()"
                class="h-16 text-sm bg-white text-black hover:bg-gray-100 border border-gray-300 rounded font-medium transition-colors flex flex-col items-center justify-center gap-1">
                <span class="text-center leading-tight px-1">Falcon: Команды</span>
              </button>
              ...
              <button *ngIf="sendDishCompleted"
                ...
                class="h-16 text-sm bg-white text-black hover:bg-gray-100 border border-gray-300 rounded font-medium transition-colors flex flex-col items-center justify-center gap-1">
                <span class="text-center leading-tight px-1">Falcon: Повторить отправку</span>
              </button>
              <!-- Пустые плейсхолдеры для визуализации сетки 3×6 -->
      ...
      <div *ngIf="activeModal === 'falcon_commands_order'"
           class="fixed inset-0 z-50 flex items-center justify-center bg-black/60"
           (click)="closeDialog()">
        <div class="bg-[#3a3a3a] rounded-lg w-full max-w-md mx-4 overflow-hidden animate-slide-up"
             (click)="$event.stopPropagation()">
...
export class FalconPosScreenComponent implements OnInit, OnDestroy {
  private route = inject(ActivatedRoute);
  private router = inject(Router);

  // --- State из v1.0 + v1.1 (перенос) ---
  activeModal: FalconModalType = null;
  currentContext: FalconContextType = 'order';
  ...
  notifications: FalconNotification[] = [];
  ...
  pendingAfterRobotStatus: FalconModalType | null = null;
  ...
  // v1.9 M2 (П-11): Плейсхолдеры для сетки плагинов из заказа (2 кнопки Falcon → 16 плейсхолдеров)
  ...
  get activeNotifications(): FalconNotification[] {
    ...
    this.route.queryParams.subscribe(params => {
      ...
      if (params['modal']) {
        ...
        if (params['modal'] === 'cleanup_multi_select') {
          ...
          this.activeModal = params['modal'] as FalconModalType;
        }
      }
  ...
  // v1.8: Из «Плагины» → «Falcon: Статус роботов»
  onPluginRobotStatus(): void {
    this.openRobotStatus();
  }

  // v1.8: Из «Плагины» → «Falcon: Команды»
  onPluginFalconCommands(): void {
    this.activeModal = 'falcon_commands';
  }
  ...
  // v1.9: Из «Плагины (заказ)» → «Falcon: Команды»
  onPluginFalconCommandsOrder(): void {
    this.activeModal = 'falcon_commands_order';
  }
  ...
  pushNotification(title: string, message: string): void {
    const notif: FalconNotification = {
      id: 'notif-' + Date.now(),
      type: 'error',
      title,
      message,
  ...
  dismissNotificationWithTracking(notification: FalconNotification): void {
  ...
  trackNotif(_: number, notif: FalconNotification): string {
    return notif.id;
  }
