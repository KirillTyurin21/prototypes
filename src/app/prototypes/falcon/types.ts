/** Типы для прототипа «Falcon — Управление роботами (Front)» v1.1 */

/** Контекст вызова плагина: из заказа или с главного экрана */
export type FalconContextType = 'order' | 'main';

export interface FalconRobot {
  robot_id: string;
...
export type FalconModalType =
  ...
  | 'plugins_menu'                   // v1.8: Окно «Плагины» (кнопки плагинов) — Главный экран
  | 'falcon_commands'                  // v1.8: Окно «Команды роботам» (Уборка + Маркетинг) — Главный экран
  // v1.9: Контекст «Из заказа» — Дополнения → Плагины → Команды
  | 'plugins_menu_order'             // v1.9: Окно «Плагины» из контекста заказа
  | 'falcon_commands_order'            // v1.9: Окно «Команды роботам» из заказа (4 кнопки)
...
export interface FalconDialogEntry {
  id: string;
  modalType: FalconModalType;
  name: string;
  description: string;
  icon: string;
  iconColor: string;
...
export interface CatalogCell {
  ...
  modalType?: FalconModalType;         // Какую модалку открыть (одиночные)
...
export interface ScenarioStep {
  modal: FalconModalType;              // Модалка для открытия
