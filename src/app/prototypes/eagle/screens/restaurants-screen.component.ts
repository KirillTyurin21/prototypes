...
@Component({
  ...
  template: `
    ...
    <div class="border-b border-gray-200 bg-white px-6 py-4 shrink-0">
      <h1 class="text-lg font-semibold text-gray-900">Настройки Eagle</h1>
    </div>
    ...
    <div class="bg-gray-50 p-6 flex-1 overflow-y-auto">
      ...
      <div *ngIf="!isLoading && !hasError && restaurants.length === 0" class="flex items-center justify-center h-64" role="status">
        <p class="text-gray-400 text-sm">Нет ресторанов, доступных для настройки Eagle</p>
      </div>

      <!-- WITH DATA -->
      <ng-container *ngIf="!isLoading && !hasError && restaurants.length > 0">

        <!-- v1.11 N1: NeConnectionCard УДАЛЕНА — credentials являются системными константами -->
        ...
        <div *ngIf="filteredRestaurants.length > 0" class="animate-fade-in">
          <div class="bg-white border border-gray-200 rounded-lg overflow-hidden">
            <table class="w-full">
              <thead>
                <tr class="bg-gray-50">
                  ...
                  <th class="px-4 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider"
                      title="Общее количество зарегистрированных роботов Eagle в этом ресторане (не считая удалённых)">
                    Всего роботов
                  </th>
                  <th class="px-4 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider"
                      title="Количество роботов, которые сейчас подключены к облаку Eagle и доступны для выполнения задач">
                    Активных роботов
                  </th>
                  <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider"
                      title="Агрегированный статус готовности интеграции Eagle: настроены ли роботы, маппинг столов и сценарии">
                    Статус настройки
                  </th>
              ...
              <tbody>
                  <!-- v1.11 N3: Строки всегда кликабельны (soft-block удалён) -->
                <tr
...
export class RestaurantsScreenComponent implements OnInit {
  ...
  getStatusTooltip(status: Restaurant['setup_status']): string {
    switch (status) {
      case 'configured': return 'Все настройки интеграции Eagle завершены: роботы зарегистрированы, маппинг столов выполнен, сценарии активированы';
      case 'partial': return 'Настройка интеграции Eagle выполнена частично: часть настроек ещё не завершена (проверьте роботов, маппинг столов и сценарии)';
      case 'not_configured': return 'Интеграция Eagle не настроена: роботы не зарегистрированы, маппинг столов и сценарии не настроены';
    }
  }

  // v1.11 N3: Переход всегда доступен (soft-block удалён)
  selectRestaurant(restaurant: Restaurant): void {
    this.parent.setRestaurantContext(restaurant);
    this.router.navigate(['/prototype/eagle/robots']);
  }
}
