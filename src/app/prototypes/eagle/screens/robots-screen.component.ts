...
import { EaglePrototypeComponent } from '../eagle-prototype.component';
  ...
  template: `
    ...
    <div class="border-b border-gray-200 bg-white px-6 py-4 shrink-0">
      ...
      <nav class="flex items-center gap-1.5 text-sm mt-2" aria-label="Breadcrumb">
        <a class="text-gray-400 hover:text-blue-600 hover:underline cursor-pointer transition-colors" (click)="goBack()">Настройки Eagle</a>
        ...
        <lucide-icon name="chevron-right" [size]="14" class="text-gray-300"></lucide-icon>
        <span class="text-gray-900 font-medium">Роботы Eagle</span>
      </nav>
    </div>
        ...
        <table class="w-full bg-white border border-gray-200 rounded-lg overflow-hidden">
          <thead>
            <tr class="bg-gray-50">
              ...
              <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider" title="Серийный номер робота в системе Eagle. Неизменяемый, задаётся при регистрации">ID робота</th>
              <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider" title="Текущий статус подключения робота к облаку Eagle. online — робот доступен, offline — нет связи, ошибка — сбой подключения">Статус</th>
              <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider" title="Название активной карты зала робота в системе Eagle. Настраивается инженером NE при установке">Активная карта</th>
                ...
                <span class="inline-flex items-center gap-1.5">
                  <lucide-icon
                    *ngIf="robot.connection_status === 'online'"
                    ...
                    title="Робот подключён к облаку Eagle и доступен для задач"
                  ></lucide-icon>
                  <lucide-icon
                    *ngIf="robot.connection_status === 'offline'"
                    ...
                    [size]="16"
                    class="text-gray-300"
                    title="Робот не подключён к облаку Eagle"
                  ></lucide-icon>
                  <lucide-icon
                    *ngIf="robot.connection_status === 'error'"
                    ...
                    title="Ошибка подключения к облаку Eagle"
                  ></lucide-icon>
    ...
    <ui-modal [open]="addModalOpen" title="Регистрация робота" size="md" (modalClose)="closeAddModal()">
      <div class="space-y-4">
        ...
        <div>
          ...
          <ui-input
            [(value)]="newRobotId"
            placeholder="Серийный номер Eagle"
            hint="Серийный номер робота из системы Eagle"
            [error]="formErrors['id']"
...
export class RobotsScreenComponent implements OnInit {
  ...
  parent = inject(EaglePrototypeComponent);
  ...
  goBack(): void {
    ...
    this.router.navigate(['/prototype/eagle']);
  }

  // --- Helpers ---
  private persistRobots(): void {
    this.storage.save('eagle', 'robots', this.robots);
  }
