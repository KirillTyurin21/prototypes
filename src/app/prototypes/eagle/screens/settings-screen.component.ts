              ...
              <div class="bg-blue-50 border border-blue-200 rounded-lg p-4" role="note">
                <div class="flex items-start gap-3">
                  ...
                  <p class="text-xs text-gray-600">
                    ...
                    Определение факта загрузки посуды недоступно (ограничение API Eagle).
                  </p>
                </div>
              </div>
            </div>
...
export class SettingsScreenComponent implements OnInit {
  ...
  ngOnInit(): void {
    setTimeout(() => {
      ...
      this.settings = this.storage.load('eagle', 'settings', getInitialSettings());
  ...
  private saveInternal(): void {
    ...
    this.savedQrPhraseSuccess = this.settings.qr_payment.phrase_success;
    this.savedQrPhraseFailure = this.settings.qr_payment.phrase_failure;
    this.storage.save('eagle', 'settings', this.settings);
  }
