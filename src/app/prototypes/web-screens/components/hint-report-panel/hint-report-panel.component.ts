...
@Component({
    ...
    <div class="report-panel" [class.panel-open]="open" *ngIf="open">
      ...
      <div class="panel-filter">
        ...
        <div class="period-toggle">
          <button
            class="period-btn"
            [class.period-active]="selectedPeriod === 'week'"
            (click)="onPeriodChange('week')"
          >Неделя</button>
          <button
            class="period-btn"
            [class.period-active]="selectedPeriod === 'month'"
      ...
      <div class="panel-body">
        ...
        <div *ngIf="state === 'empty'" class="panel-empty">
          <div class="kpi-grid">
            <div class="kpi-card kpi-zero">
              <span class="kpi-value">0</span>
              <span class="kpi-label">Показов</span>
            </div>
            <div class="kpi-card kpi-zero">
              <span class="kpi-value">0</span>
              <span class="kpi-label">Срабатываний</span>
            </div>
            <div class="kpi-card kpi-zero">
              <span class="kpi-value">0%</span>
        ...
        <div *ngIf="state === 'normal' && data" class="panel-data">
          <div class="kpi-grid">
            <div class="kpi-card">
              <span class="kpi-value">{{ data.shows }}</span>
              <span class="kpi-label">Показов</span>
            </div>
            <div class="kpi-card">
              <span class="kpi-value">{{ data.conversions }}</span>
              <span class="kpi-label">Срабатываний</span>
            </div>
            <div class="kpi-card">
              <span class="kpi-value">{{ data.conversionPercent }}%</span>
              <span class="kpi-label">Конверсия</span>
            </div>
          </div>
        </div>
  ...
  styles: [`
    ...
    .panel-overlay {
      position: absolute; inset: 0; background: rgba(0,0,0,0.25); z-index: 50;
      animation: fadeIn 0.2s ease-out;
    }
    @keyframes fadeIn { from { opacity: 0; } to { opacity: 1; } }

    .report-panel {
      position: absolute; top: 0; right: 0; bottom: 0; width: 100%; max-width: 720px;
    ...
    .panel-back-btn, .panel-close-btn {
      ...
      background: transparent; color: #616161; cursor: pointer; flex-shrink: 0;
      transition: background 0.15s;
    }
    .panel-back-btn:hover, .panel-close-btn:hover { background: #f5f5f5; color: #212121; }
    .panel-title {
      flex: 1; font-size: 18px; font-weight: 500; color: #212121; margin: 0;
    ...
    .panel-filter {
      ...
      padding: 12px 20px; border-bottom: 1px solid #eeeeee;
      flex-shrink: 0;
    }
    ...
    .period-btn {
      padding: 6px 16px; border: none; background: #fff; color: #616161;
    ...
    .kpi-value {
      font-size: 32px; font-weight: 700; color: #212121; line-height: 1;
    }
    .kpi-label {
      font-size: 13px; font-weight: 500; color: #757575; text-transform: uppercase; letter-spacing: 0.3px;
    }
    ...
    .skeleton-value {
      width: 80px; height: 36px; background: #eeeeee; border-radius: 4px;
      margin: 0 auto; animation: pulse 1.5s infinite;
    }
    .skeleton-label {
      width: 100px; height: 14px; background: #eeeeee; border-radius: 4px;
      margin: 4px auto 0; animation: pulse 1.5s infinite;
    }
    ...
    .retry-btn {
      ...
      padding: 8px 16px; border: 1px solid #e0e0e0; border-radius: 4px;
      background: #fff; color: #1976d2; font-size: 13px; font-family: Roboto, sans-serif;
    ...
    .empty-text {
      text-align: center; font-size: 14px; color: #9e9e9e; margin: 8px 0 0;
    }
  `],
})
export class HintReportPanelComponent implements OnInit, OnChanges {
  ...
  loadData(): void {
    ...
    this.loadTimer = setTimeout(() => {
      ...
      if (report.shows === 0) {
        this.data = { shows: 0, conversions: 0, conversionPercent: 0 };
        this.state = 'empty';
      } else {
        this.data = report;
        this.state = 'normal';
...
/** Generate mock report data based on hint ID and period */
function generateReportData(hintId: number, period: 'week' | 'month'): ReportData {
  // Use hintId as seed for deterministic variance
