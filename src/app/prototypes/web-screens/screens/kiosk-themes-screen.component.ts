...
export class KioskThemesScreenComponent {
  ...
  onRowClick(item: ArrivalsThemeListItem): void {
    if (item.itemType === 'theme') {
      this.router.navigate(['/prototype/web-screens/kiosk-theme-editor', item.id]);
    }
  }

  onEdit(item: ArrivalsThemeListItem, event: MouseEvent): void {
  ...
  onDelete(item: ArrivalsThemeListItem, event: MouseEvent): void {
    event.stopPropagation();
  ...
  confirmDelete(): void {
    if (this.deleteTarget) {
      ...
      this.storage.save('web-screens', 'kiosk-list', this.items);
      this.deleteTarget = null;
    }
  }

  onAdd(): void {
    const newId = Math.max(0, ...this.items.filter(i => i.itemType === 'theme').map(i => i.id)) + 1;
    const newTheme: ArrivalsThemeListItem = {
      id: newId,
      name: `Новая тема #${newId}`,
      itemType: 'theme',
      resolution: '1024x768',
      createdBy: 'Моя',
    };
    this.items = [...this.items, newTheme];
    this.storage.save('web-screens', 'kiosk-list', this.items);
    this.router.navigate(['/prototype/web-screens/kiosk-theme-editor', newId]);
  }
}
