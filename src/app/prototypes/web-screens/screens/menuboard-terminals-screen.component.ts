...
@Component({
  selector: 'app-menuboard-terminals-screen',
  standalone: true,
...
export class MenuboardTerminalsScreenComponent {
  ...
  getHintsSummary(terminal: CSTerminalV2): string {
    ...
    return terminal.hintIds.length + ' vibrano';
  }
  ...
  getTerminalWord(n: number): string {
    ...
    if (m100 >= 11 && m100 <= 19) return 'terminalov';
    if (m10 === 1) return 'terminal';
    if (m10 >= 2 && m10 <= 4) return 'terminala';
    return 'terminalov';
  }
  save(): void { this.dataService.updateRestaurants(this.dataService.restaurants); this.showToastMessage('Nastroiki terminalov sohraneni', 'success'); }
  async sendSettings(): Promise<void> {
    ...
    this.isSending = true;
    this.showToastMessage('Otpravka nastroek na ' + ids.length + ' ' + this.getTerminalWord(ids.length) + '...', 'success');
    try { await this.dataService.sendSettings(ids); this.showToastMessage('Nastroiki uspeshno otpravleni na ' + ids.length + ' ' + this.getTerminalWord(ids.length), 'success'); this.selectedTerminals.clear(); }
    catch { this.showToastMessage('Oshibka pri otpravke nastroek', 'error'); }
  ...
  async requestScreenshot(terminal: CSTerminalV2, restaurant: CSRestaurant): Promise<void> {
    ...
    this.showToastMessage('Poluchenie skrinshota s <<' + terminal.name + '>>...', 'success');
    try { const s = await this.dataService.requestScreenshot(terminal.id); if (s) this.screenshotModal = s; }
    catch (e: any) { this.showToastMessage(e?.message || 'Oshibka polucheniya skrinshota', 'error'); }
