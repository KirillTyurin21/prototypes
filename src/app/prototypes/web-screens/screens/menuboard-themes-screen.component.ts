...
@Component({
  selector: 'app-menuboard-themes-screen',
  standalone: true,
                  ...
                  <div class="name-cell">
                    ...
                    <span
                      *ngIf="item.itemType === 'theme' && themeHasPremium(item.id)"
                      class="premium-badge"
                      title="Платный контент: Область контролов">
  ...
  styles: [`
    ...
    .btn-outlined-blue { background: transparent; border: 2px solid var(--dt-brand-accent); color: var(--dt-brand-accent); }
    .btn-outlined-blue:hover { background: var(--dt-brand-accent-lighter); }
    ...
    .themes-table thead { border-bottom: 2px solid var(--dt-stroke-default); }
    .themes-table th {
    ...
    .themes-table td { padding: 12px; border-bottom: 1px solid var(--dt-stroke-disable); }
...
export class MenuboardThemesScreenComponent {
  private router = inject(Router);
  private storage = inject(StorageService);

  items: ArrivalsThemeListItem[] = this.storage.load('web-screens', 'menuboard-list', [...MOCK_ARRIVALS_LIST]);
  deleteTarget: ArrivalsThemeListItem | null = null;

  /** Тема содержит платный элемент «Область контролов» (для метки платного контента) */
  themeHasPremium(themeId: number): boolean {
    const themes: any[] = this.storage.load('web-screens', 'menuboard-themes', []);
    const theme = themes.find(t => t.id === themeId);
    return !!theme && theme.elements?.some((e: any) => e.type === 'area');
  }

  onRowClick(item: ArrivalsThemeListItem): void {
    if (item.itemType === 'theme') {
      this.router.navigate(['/prototype/web-screens/menuboard-theme-editor', item.id]);
    }
  }
  ...
  onAdd(): void {
    this.router.navigate(['/prototype/web-screens/menuboard-theme-editor', 'new']);
  }
  ...
  confirmDelete(): void {
    if (this.deleteTarget) {
      ...
      this.storage.save('web-screens', 'menuboard-list', this.items);
