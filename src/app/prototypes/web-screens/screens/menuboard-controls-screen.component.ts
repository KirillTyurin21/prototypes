...
import { MOCK_ARRIVALS_CONTROLS_LIST } from '../data/mock-data';
import { ArrivalsControlListItem } from '../types';

@Component({
  selector: 'app-menuboard-controls-screen',
  standalone: true,
  imports: [CommonModule, IconsModule, UiConfirmDialogComponent],
  template: `
    <div class="controls-screen">
      <div class="page-header">
        <h1 class="page-title">Контролы доски меню</h1>
        <div class="header-actions">
          <button class="btn-header btn-outlined-orange">СОЗДАТЬ ПАПКУ</button>
      ...
      <div class="content-layout">
        <div class="table-area">
          <table class="controls-table">
            ...
            <tbody>
              <tr *ngFor="let item of items" class="control-row" (click)="onRowClick(item)">
                <td class="col-name">
                  <div class="name-cell">
                    <lucide-icon
                ...
                <td class="col-actions">
                  <div class="actions-cell">
                    ...
                    <button *ngIf="item.itemType === 'control'" class="icon-btn" title="Предпросмотр"
                      (click)="$event.stopPropagation()">
        ...
        <div class="preview-area">
          <span class="preview-hint">Кликните на контрол для предпросмотра</span>
        </div>
      </div>
  ...
  `,
  styles: [`
    .controls-screen { animation: fadeIn 0.2s ease-out; }
    ...
    .btn-add { background: transparent; border: 2px solid var(--dt-text-secondary); color: var(--dt-text-primary); }
    .btn-add:hover { background: var(--dt-surface-hover); }
    ...
    .preview-area {
      ...
      padding-top: 16px; color: var(--dt-text-disable); font-size: 14px;
    }

    .controls-table { width: 100%; border-collapse: collapse; font-size: 14px; font-family: Roboto, sans-serif; }
    .controls-table thead { border-bottom: 2px solid var(--dt-stroke-default); }
    .controls-table th {
      text-align: left; padding: 10px 12px; font-weight: 500;
      color: var(--dt-text-secondary); font-size: 13px; text-transform: uppercase; letter-spacing: 0.3px;
    }
    .controls-table td { padding: 12px; border-bottom: 1px solid var(--dt-stroke-disable); }
    .control-row { cursor: pointer; transition: background 0.15s; }
    .control-row:hover { background: var(--dt-surface-hover); }
    ...
    .col-resolution { width: 160px; color: var(--dt-text-secondary); }
    .col-created { width: 100px; color: var(--dt-text-secondary); }
    ...
    .icon-btn {
      ...
      background: transparent; color: var(--dt-text-disable); cursor: pointer; transition: all 0.15s;
    }
    .icon-btn:hover { background: var(--dt-stroke-default); color: var(--dt-text-primary); }

    .preview-hint { text-align: center; line-height: 1.5; }
  `],
})
export class MenuboardControlsScreenComponent {
  ...
  onRowClick(item: ArrivalsControlListItem): void {
    if (item.itemType === 'control') {
      this.router.navigate(['/prototype/web-screens/menuboard-control-editor', item.id]);
    }
  }

  onEdit(item: ArrivalsControlListItem, event: Event): void {
  ...
  onAdd(): void {
    this.router.navigate(['/prototype/web-screens/menuboard-control-editor', 'new']);
  }

  onDelete(item: ArrivalsControlListItem, event: Event): void {
    event.stopPropagation();
