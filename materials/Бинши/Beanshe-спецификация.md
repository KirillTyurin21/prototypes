...
- Кнопка "Отменить" в UI плагина заблокирована (отмена через стандартный UI iikoFront не затрагивается, см. раздел 3.5)
...
| 3 | Покупатель приходит, бариста нажимает "Закрыть" в стандартном UI iikoFront | Плагин перехватывает событие через подписку `OrderChanged`, вызывает PayOrder - заказ автоматически закрывается в iikoFront | Заказ в статусе Closed в iiko |
...
== Закрытие (стандартный UI iikoFront) ==
...
| Ошибка PayOrder (SDK exception) | Плагин логирует ошибку; заказ остается открытым в iikoFront; complete в Beanshe не отправляется; требуется ручное закрытие |
...
Бариста нажимает "Закрыть" в стандартном UI iikoFront. Плагин обнаруживает это через подписку `INotificationService.OrderChanged`. К этому моменту оплата уже зафиксирована через AddExternalFiscalizedPaymentItem (сценарий 4.4.1.1) и покрывает полную сумму заказа. Плагин программно вызывает PayOrder - заказ автоматически закрывается в iikoFront и переходит в статус Closed. Ручное нажатие "Оплатить" баристой не требуется. Закрытие заказа самовывоза Beanshe всегда означает выдачу клиенту (complete).
...
Если PayOrder выбрасывает исключение (например, оплата не покрывает сумму, заказ уже закрыт), плагин логирует ошибку. Заказ остается открытым в iikoFront, complete в Beanshe не отправляется.
...
| Шаг | Действие пользователя | Реакция системы | Результат |
|:---:|----------------------|-----------------|----------|
| 1 | Бариста нажимает "Отменить" | Плагин отправляет cancel в Beanshe | Статус "Отменен", деньги возвращаются |
...
| Ситуация | Реакция системы |
|----------|----------------|
| Попытка отмены после "Принят" | Кнопка "Отменить" в UI плагина заблокирована. Отмена через стандартные механизмы iikoFront не ограничивается (см. раздел 3.5) |

##### Детальная логика обработки шагов

**Шаг 1. Отмена заказа**

Бариста нажимает "Отменить" - плагин сразу отправляет запрос отмены без выбора причины.
...
| 4 | Закрытие заказа в iiko выполняется автоматически плагином (вызов PayOrder при обнаружении закрытия через `OrderChanged` или при нажатии "Выдать" в диалоге "Не забрали") |
...
| 2a | Бариста нажимает "Выдать" в диалоге плагина | Плагин вызывает PayOrder - заказ закрывается в iiko, затем плагин отправляет complete | Заказ завершен |
| 2b | Бариста нажимает "Не забрали" в диалоге плагина | Плагин вызывает PayOrder - заказ закрывается в iiko, затем плагин отправляет discard | Заказ списан |
...
Полное описание логики PayOrder - в шаге 3 сценария 4.2.1.2.

*Результат: заказ закрыт в iiko, Beanshe уведомлен*

**Шаг 2b. Списание заказа**

Бариста нажимает "Не забрали" в диалоге. Оплата уже зафиксирована (AddExternalFiscalizedPaymentItem, сценарий 4.4.1.1). Плагин вызывает PayOrder - заказ закрывается в iikoFront. После подтверждения Closed плагин отправляет `POST /api/v2/barista/orders/{id}/discard/`.
...
Статусы приготовления ("Готовится", "Готов") и закрытие заказа управляются баристой через стандартный UI iikoFront (кнопки "Готовить", "Приготовлен", "Закрыть"). Плагин мониторит эти события через подписку `INotificationService.DeliveryOrderChanged` и автоматически отправляет соответствующие уведомления в Beanshe Backend (preparing, ready, complete). Кнопок для этих действий в UI плагина нет.
